...
export const DEFAULTS = Object.seal({
  ...
  RESOURCE_LINK_SELECTOR: 'head link[href]',
  EXTRACT_HEADERS: [
    'cache-control',
    'content-length',
    'content-type',
...
export class SeoCrawler {
  ...
  /**
   *
   * @param {SeoCrawlerOptions} options
   * @type {import('./progress.js').Progress}
   */
  constructor(options, progress) {
    this.#options = options;
    this.#progress = progress;
    const parsedStart = new URL(options.startUrl);
  ...
  crawl() {
    this.#progress.start(`Starting crawl of ${this.#options.startUrl}`);
    ...
    crawler.queue([this.#options.startUrl, this.#options.startUrl]);
  ...
  handleResponse(res, crawler) {
    ...
    if (this.shouldParseUrl(url)) {
      ...
      if ($) {
        ...
        this.extractResourceLinks($, crawler, url).forEach((rl) =>
          urlInfo.outLinks.push(rl),
        );
        this.extractMedia($, crawler, url).forEach((rl) =>
          urlInfo.outLinks.push(rl),
        );
      }
    }
  ...
  extractMedia($, crawler, url) {
    ...
    $(this.#options.mediaLinkSelector || DEFAULTS.MEDIA_LINK_SELECTOR).each(
      (_idx, el) => {
        try {
          const { type, alt } = el.attribs;
          let { src } = el.attribs;
          if (!src || src.startsWith('data:')) {
            return;
          }
          src = this.resolveUrl(src);
          if (this.isIgnored(src)) {
            return;
          }
          const linkInfo = this.getUrlInfo(src);
          ...
          if (!this.#encountered.has(src)) {
            crawler.queue({ uri: src, method: 'HEAD' });
          }
        } catch (err) {
          this.#progress.log(
            'WARN',
            `Failed to extract media from ${url} err: ${err}`,
          );
        }
      },
    );
  ...
  extractLinks($, crawler, url) {
    ...
    $(this.#options.linkSelector || DEFAULTS.LINK_SELECTOR).each((_idx, el) => {
      try {
        ...
        const linkInfo = this.getUrlInfo(href);
        linkInfo.inLinks.push({
          source: url,
          title: title?.trim(),
          text: text?.trim(),
        ...
        if (!this.#encountered.has(href)) {
          crawler.queue(href);
        }
      } catch (err) {
        this.#progress.log(
          'WARN',
          `Failed extract link from ${url} err: ${err}`,
        );
      }
    });
    return links;
  ...
  extractResourceLinks($, crawler, url) {
    ...
    $(
      ...
      try {
        ...
        const linkInfo = this.getUrlInfo(href);
        linkInfo.inLinks.push({ source: url, rel, type: 'resource' });
        links.push({ target: href, rel, type: 'resource' });
        if (!this.#encountered.has(href)) {
          crawler.queue(href);
        }
      } catch (err) {
        this.#progress.log(
          'WARN',
          `Failed to extract resource link from ${url} err: ${err}`,
        );
      }
    });
    return links;
  ...
  resolveUrl(uri) {
    if (uri.startsWith('//')) {
      ...
      return new URL(`${this.#baseUrl}${uri}`).href;
    }
